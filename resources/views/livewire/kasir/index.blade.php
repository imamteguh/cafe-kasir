...
class extends Component {    
    ...
    public $drawerKey, $strukModalKey;
    public bool $drawer = false;
    public bool $strukModal = false;
    ...
    public function addToCart(int $productId): void
    {
        ...
        $this->success('Product added to cart', position: 'bottom-end');
    }
    ...
    #[On('open-struk-modal')]
    public function open_struk_modal($invoice): void
    {
        $this->strukModalKey = uniqid();
        $this->invoice = $invoice;
        $this->strukModal = true;
    }

    #[On('close-struk-modal')]
    public function close_struk_modal(): void
    {
        $this->strukModalKey = null;
        $this->strukModal = false;
    }
}; ?>
...
<div>
    ...
    <x-drawer :key="$drawerKey" wire:model="drawer" class="w-11/12 md:w-8/12 lg:w-1/3" right without-trap-focus>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        @foreach ($products as $product)
            <x-card title="{{ $product->name }}" separator shadow>
                ...
                <x-slot:menu>
                    <x-button wire:click.prevent="addToCart({{ $product->id }})" icon="o-plus" class="btn-error btn-sm btn-soft btn-circle" spinner/>
                </x-slot:menu>
            </x-card>
        @endforeach
    </div>
    <x-pagination :rows="$products" wire:model.live="perPage" />

    <x-modal :key="$strukModalKey" wire:model="strukModal" class="backdrop-blur">
        @livewire('kasir.struk', ['invoice' => $invoice], key($strukModalKey))
    </x-modal>
